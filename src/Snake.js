...
module.exports = async (options) => {
	...
	if (!options.embed.title) {
		options.embed.title = 'Snake | Sudo Development';
	}
	if (typeof options.embed.title !== 'string') {
		throw new TypeError('Sudo Error: title must be a string.');
	}
	...
	if (typeof options.embed.description !== 'string') {
		throw new TypeError('Sudo Error: description must be a string.');
	}

	if (!options.embed.color) options.embed.color = functions.randomHexColor();
	if (typeof options.embed.color !== 'string') {
		throw new TypeError('Sudo Error: color must be a string.');
	}
	...
	if (typeof options.emojis !== 'object') {
		throw new TypeError('Sudo Error: emojis must be an object.');
	}

	if (!options.emojis.empty) options.emojis.empty = '⬛';
	if (typeof options.emojis.empty !== 'string') {
		throw new TypeError('Sudo Error: empty emoji must be an emoji.');
	}

	if (!options.emojis.snakeBody) options.emojis.snakeBody = '🟩';
	if (typeof options.emojis.snakeBody !== 'string') {
		throw new TypeError('Sudo Error: snakeBody emoji must be an emoji.');
	}

	if (!options.emojis.snakeHead) options.emojis.snakeHead = '🟨';
	if (typeof options.emojis.snakeHead !== 'string') {
		throw new TypeError('Sudo Error: snakeHead emoji must be an emoji.');
	}

	if (!options.emojis.snakeTail) options.emojis.snakeTail = '🟧';
	if (typeof options.emojis.snakeTail !== 'string') {
		throw new TypeError('Sudo Error: snakeTail emoji must be an emoji.');
	}

	if (!options.emojis.food) options.emojis.food = '🍎';
	if (typeof options.emojis.food !== 'string') {
		throw new TypeError('Sudo Error: food emoji must be an emoji.');
	}

	if (!options.emojis.up) options.emojis.up = '⬆️';
	if (typeof options.emojis.up !== 'string') {
		throw new TypeError('Sudo Error: up emoji must be an emoji.');
	}

	if (!options.emojis.right) options.emojis.right = '⬅️';
	if (typeof options.emojis.right !== 'string') {
		throw new TypeError('Sudo Error: right emoji must be an emoji.');
	}

	if (!options.emojis.down) options.emojis.down = '⬇️';
	if (typeof options.emojis.down !== 'string') {
		throw new TypeError('Sudo Error: down emoji must be an emoji.');
	}

	if (!options.emojis.left) options.emojis.left = '➡️';
	if (typeof options.emojis.left !== 'string') {
		throw new TypeError('Sudo Error: left emoji must be an emoji.');
	}
	...
	const id4 = functions.getRandomID(20);

	const id5 = functions.getRandomID(20);

	const id6 = functions.getRandomID(20);

	const id7 = functions.getRandomID(20);
	...
	const lock1 = new Discord.MessageButton()
		.setLabel('\u200b')
		.setStyle('SECONDARY')
		.setCustomId(id1)
		.setDisabled();
	let w = new Discord.MessageButton()
		...
		.setStyle('PRIMARY')
		.setCustomId(id2);
	const lock2 = new Discord.MessageButton()
		.setLabel('\u200b')
		.setStyle('SECONDARY')
		.setCustomId(id7)
		.setDisabled();
