...
module.exports = async (options) => {
	...
	if (!options.embed.title) options.embed.title = 'Quick Click | Sudo Development';
	...
	if (typeof options.waitMessage !== 'string') throw new TypeError('Sudo Error: waitMessage must be a string.');
	...
	if (typeof options.winMessage !== 'string') throw new TypeError('Sudo Error: startMessage must be a string.');

	if (!options.loseMessage) options.loseMessage = 'No one pressed the button in time. So, I dropped the game!';
	if (typeof options.loseMessage !== 'string') throw new TypeError('Sudo Error: startMessage must be a string.');

	if (!options.emoji) options.emoji = '👆';
	if (typeof options.emoji !== 'string') throw new TypeError('Sudo Error: emoji must be a string.');

	if (!options.ongoingMessage) options.ongoingMessage = 'A game is already runnning in <#{{channel}}>. You can\'t start a new one!';
	if (typeof options.ongoingMessage !== 'string') throw new TypeError('Sudo Error: ongoingMessage must be a string.');
	...
	setTimeout(async function() {
		...
		for (let i = 0; i < 24; i++) {
			buttons.push(
				new Discord.MessageButton().setDisabled().setLabel('\u200b').setStyle('PRIMARY').setCustomId(functions.getRandomID(20)),
			);
		}

		buttons.push(
			new Discord.MessageButton().setStyle('PRIMARY').setEmoji(options.emoji).setCustomId('CORRECT'),
		);

		functions.shuffleArray(buttons);

		for (let i = 0; i < 5; i++) { rows.push(new Discord.MessageActionRow()); }

		rows.forEach((row, i) => { row.addComponents(buttons.slice(0 + i * 5, 5 + i * 5)); });

		const _embed = new Discord.MessageEmbed()
			.setTitle(options.embed.title)
			.setColor(options.embed.color)
			.setFooter(options.embed.footer)
			.setDescription(options.startMessage.replace('{{time}}',functions.convertTime(options.time),),
			);
