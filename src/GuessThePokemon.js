...
module.exports = async (options) => {
	...
	if (!options.embed.title) options.embed.title = 'Guess The Pokémon | Sudo Development';
	if (typeof options.embed.title !== 'string') throw new TypeError('Sudo Error: embed title must be a string.');

	if (!options.embed.description) options.embed.description = '**Type:**\n{{type}}\n\n**Abilities:**\n{{abilities}}\n\nYou only have **{{time}}** to guess the pokémon.';
	...
	if (!options.incorrectMessage) options.incorrectMessage = 'No {{author}}! The pokémon isn\'t `{{answer}}`';
	...
	const btn1 = new Discord.MessageButton().setStyle('DANGER').setLabel(options.buttonText).setCustomId(id);
	...
	collector.on('collect', async (msg) => {
		if (msg.content.toLowerCase() === data.data.name.toLowerCase()) {
			const _embed = new Discord.MessageEmbed()
				...
				.setDescription(
					options.winMessage
						.replace('{{answer}}',data.data.name.charAt(0).toUpperCase() + data.data.name.slice(1),)
						.replace('{{time}}',functions.convertTime(Date.now() - gameCreatedAt),),
				)
				.setColor(options.embed.color)
