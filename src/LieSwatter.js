...
module.exports = async (options) => {
	...
	if (!options.embed.title) options.embed.title = 'Lie Swatter | Sudo Development';
	...
	if (typeof options.buttons.true !== 'string') throw new TypeError('Sudo Error: true buttons text must be a string.');

	if (!options.buttons.lie) options.buttons.lie = 'Lie';
	if (typeof options.buttons.lie !== 'string') throw new TypeError('Sudo Error: lie buttons text must be a string.');
	...
	winningID = id1;answer = options.buttons.true;}
	else winningID = id2; answer = options.buttons.lie;

	const btn1 = new Discord.MessageButton().setStyle('PRIMARY').setLabel(options.buttons.true).setCustomId(id1);
	const btn2 = new Discord.MessageButton().setStyle('PRIMARY').setLabel(options.buttons.lie).setCustomId(id2);
	...
	const gameCollector = think.createMessageComponentCollector({filter: (fn) => fn});
	...
	gameCollector.on('collect', async (button) => {
		...
		if (button.customId === winningID) {
			...
			const lostEmbed = new Discord.MessageEmbed()
				.setDescription(`${options.loseMessage.replace('{{answer}}', answer)}`,)
				.setColor(options.embed.color)
				.setFooter(options.embed.footer);
