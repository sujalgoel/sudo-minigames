...
module.exports = async (options) => {
	...
	if (!options.embed.title) options.embed.title = 'Fast Type | Sudo Development';
	if (typeof options.embed.title !== 'string') throw new TypeError('Sudo Error: embed title must be a string.');

	if (!options.embed.description) options.embed.description = 'You have **{{time}}** to type the below sentence.';
	...
	if (!options.sentence) options.sentence = functions.getRandomSentence(Math.floor(Math.random() * 10) + 3).toString().split(',').join(' ');
	if (typeof options.sentence !== 'string') throw new TypeError('Sudo Error: sentence must be a string');

	if (!options.winMessage) options.winMessage = 'GG, you have a wpm of **{{wpm}}** and You made it in **{{time}}**.';
	...
	if (typeof options.cancelMessage !== 'string') throw new TypeError('Sudo Error: othersMessage must be a string.');

	const id = functions.getRandomID();

	const sentence = options.sentence.toLowerCase().split(' ').map((msg) => `\`${msg.split('').join(' ')}\``).join(' ');
	...
	collector.on('collect', async (msg) => {
		if (msg.content.toLowerCase().trim() === options.sentence.toLowerCase()) {
			...
			const _embed = new Discord.MessageEmbed()
				.setDescription(options.winMessage.replace('{{time}}', functions.convertTime(time)).replace('{{wpm}}', wpm.toFixed(2)),)
				.setFooter(options.embed.footer)
				.setColor(options.embed.color);
	...
	gameCollector.on('collect', (button) => {
		if (button.user.id !== options.message.author.id) return button.reply({content: options.othersMessage.replace('{{author}}',options.message.member.id,),ephemeral: true,});
