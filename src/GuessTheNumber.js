...
module.exports = async (options) => {
	...
	if (!options.embed.title) options.embed.title = 'Guess The Number | Sudo Development';
	if (typeof options.embed.title !== 'string') throw new TypeError('Sudo Error: embed title must be a string.');

	if (!options.embed.description) options.embed.description = 'You have **{{time}}** to guess the number.';
	...
	if (typeof options.publicGame !== 'boolean') throw new TypeError('Sudo Error: publicGame must be a boolean.');

	if (!options.number) options.number = Math.floor(Math.random() * 1000);
	if (typeof options.number !== 'number') throw new TypeError('Sudo Error: number must be a number.');
	...
	if (typeof options.winMessage !== 'object') throw new TypeError('Sudo Error: winMessage must be an object.');

	if (!options.winMessage.publicGame) options.winMessage.publicGame = 'GG, The number which I guessed was **{{number}}**. <@{{winner}}> made it in **{{time}}**.\n\n__**Stats of the game:**__\n**Duration**: {{time}}\n**Number of participants**: {{totalparticipants}} Participants\n**Participants**: {{participants}}';
	if (typeof options.winMessage.publicGame !== 'string') throw new TypeError('Sudo Error: winMessage must be a string.');

	if (!options.winMessage.privateGame) options.winMessage.privateGame = 'GG, The number which I guessed was **{{number}}**. You made it in **{{time}}**.';
	if (typeof options.winMessage.privateGame !== 'string') throw new TypeError('Sudo Error: winMessage must be a string.');

	if (!options.loseMessage) options.loseMessage = 'Better luck next time! The number which I guessed was **{{number}}**.';
	if (typeof options.loseMessage !== 'string') throw new TypeError('Sudo Error: loseMessage must be a string.');

	if (!options.bigNumberMessage) options.bigNumberMessage = 'No {{author}}! My number is greater than **{{number}}**.';
	if (typeof options.bigNumberMessage !== 'string') throw new TypeError('Sudo Error: bigNumberMessage must be a string.');

	if (!options.smallNumberMessage) options.smallNumberMessage = 'No {{author}}! My number is smaller than **{{number}}**.';
	if (typeof options.smallNumberMessage !== 'string') throw new TypeError('Sudo Error: smallNumberMessage must be a string.');
	...
	if (options.publicGame) {
		...
		const gameCollector = msg.createMessageComponentCollector({filter: (fn) => fn});
		...
		collector.on('collect', async (_msg) => {
			if (!participants.includes(_msg.author.id)) participants.push(_msg.author.id);
