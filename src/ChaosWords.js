...
module.exports = async (options) => {
	...
	if (!options.embed.title) options.embed.title = 'Chaos Words | Sudo Development';
	if (typeof options.embed.title !== 'string') throw new TypeError('Sudo Error: embed title must be a string.');

	if (!options.embed.description) options.embed.description = 'You have **{{time}}** to find the hidden words in the below sentence.';
	if (typeof options.embed.description !== 'string') throw new TypeError('Sudo Error: embed color must be a string.');

	if (!options.embed.field1) options.embed.field1 = 'Sentence:';
	if (typeof options.embed.field1 !== 'string') throw new TypeError('Sudo Error: field1 must be a string.');

	if (!options.embed.field2) options.embed.field2 = 'Words Found/Remaining Words:';
	if (typeof options.embed.field2 !== 'string') throw new TypeError('Sudo Error: field2 must be a string.');

	if (!options.embed.field3) options.embed.field3 = 'Words found:';
	if (typeof options.embed.field3 !== 'string') throw new TypeError('Sudo Error: field3 must be a string.');

	if (!options.embed.field4) options.embed.field4 = 'Words:';
	if (typeof options.embed.field4 !== 'string') throw new TypeError('Sudo Error: field4 must be a string.');
	...
	if (!options.winMessage) options.winMessage = 'GG, You won! You made it in **{{time}}**.';
	...
	if (!options.wrongWordMessage) options.wrongWordMessage = 'Wrong Guess! You have **{{remaining_tries}}** tries left.';
	if (typeof options.wrongWordMessage !== 'string') throw new TypeError('Sudo Error: wrongWordMessage must be a string.');

	if (!options.correctWordMessage) options.correctWordMessage = 'GG, **{{word}}** was correct! You have to find **{{remaining}}** more word(s).';
	if (typeof options.correctWordMessage !== 'string') throw new TypeError('Sudo Error: correctWordMessage must be a string.');
	...
	if (!options.words) options.words = functions.getRandomSentence(Math.floor(Math.random() * 6) + 2);
	if (typeof options.words !== 'object') throw new TypeError('Sudo Error: words must be an array.');

	if (!options.charGenerated) options.charGenerated = options.words.join('').length - 1;
	if (typeof options.charGenerated !== 'number') throw new TypeError('Sudo Error: charGenerated must be a number.');

	if (!options.maxTries) options.maxTries = 10;
	if (typeof options.maxTries !== 'number') throw new TypeError('Sudo Error: maxTries must be a number.');
	...
	const authorId = options.message instanceof Discord.Interaction
			? options.message.user.id
			: options.message.author.id;

	const id = functions.getRandomID();
	...
	const embed = new Discord.MessageEmbed()
		...
		.setDescription(options.embed.description.replace('{{time}}',functions.convertTime(options.time)))
	...
	const game = await options.message.channel.createMessageCollector({filter: (m) => m.author.id === authorId, time: options.time });
	...
	const gameCollector = mes.createMessageComponentCollector({filter: (fn) => fn,});
