...
module.exports = async (options) => {
	...
	if (!options.embed.title) {
		options.embed.title = 'Quick Click | Sudo Development';
	}
	...
	if (typeof options.waitMessage !== 'string') {
		throw new TypeError('Sudo Error: waitMessage must be a string.');
	}
	...
	if (typeof options.emoji !== 'string') {
		throw new TypeError('Sudo Error: emoji must be a string.');
	}
		...
		for (let i = 0; i < 24; i++) {
			buttons.push(
				new Discord.MessageButton()
					...
					.setLabel('\u200b')
					.setStyle('PRIMARY')
					.setCustomId(functions.getRandomID(20)),
			);
		}
